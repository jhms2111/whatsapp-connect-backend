...
async function creditWhatsapp(
  ...
  subscriptionId?: string | null        // 👈 NOVO PARAM: id da sub
) {
  const now = new Date();
  const doc = await ConversationQuota.findOne({ username }).lean().exec();

  // novo período ou inexistente
    ...
    await ConversationQuota.findOneAndUpdate(
      { username },
      {
        ...
        $set: {
          ...
          // 👇 SALVA a assinatura (sub_...) no documento
  ...
  await ConversationQuota.updateOne(
    { username },
    {
      ...
      $set: {
        ...
        // 👇 atualiza stripeSubscriptionId se vier (mantém o antigo se não vier)
...
async function creditWebchat(
  ...
  conversationsToAdd: number,
  purchaseId?: string | null,
  subscriptionId?: string | null       // 👈 PARAM já existia
) {
  const now = new Date();
...
async function creditPackageByChannel(
  ...
  packageType: number,
  purchaseId?: string | null,
  subscriptionId?: string | null     // 👈 já existia, agora passa também pro WhatsApp
) {
  ...
  if (channel === 'whatsapp') {
    // 👇 agora envia subscriptionId também
